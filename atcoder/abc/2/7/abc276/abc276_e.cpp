...
int main()
{
    ...
    repauto(m, move1)
    {
        if (0 <= sy + m.S && sy + m.S < w && 0 <= sx + m.F && sx + m.F < h && c[sx + m.F][sy + m.S] == '.')
        {
            auto dist = G.BFS((sx + m.F) * w + sy + m.S);
            repauto(mm, move1)
            {
                if (m != mm && 0 <= sy + mm.S && sy + mm.S < w && 0 <= sx + mm.F && sx + mm.F < h)
                {
                    if (dist[(sx + mm.F) * w + sy + mm.S] != INF && dist[(sx + mm.F) * w + sy + mm.S] > 1)
                    {
                        Yes;
                        Ret;
                    }
