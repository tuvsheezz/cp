...
int main()
{
    ...
    rep(i, n)
    {
        if (dist(sx, sy, a[i][0], a[i][1]) == a[i][2] * a[i][2])
            s = i;
        if (dist(tx, ty, a[i][0], a[i][1]) == a[i][2] * a[i][2])
            t = i;
    }
    if (s == -1 || t == -1)
    {
    ...
    Graph G;
    G.init(n);
    rep(i, n) repa(j, i + 1, n)
    {
        LL dd = dist(a[i][0], a[i][1], a[j][0], a[j][1]);

        if (dd >= (a[i][2] - a[j][2]) * (a[i][2] - a[j][2]) && dd <= (a[i][2] + a[j][2]) * (a[i][2] + a[j][2]))
            G.add_edge(i, j);
    }
    auto dis = G.BFS(s);
    if (dis[t] != INF)
